...
class ProntoLIN(torch.nn.Module):
    ...
    def reg(self):
        return self.l1_reg * torch.sum(torch.abs(torch.sigmoid(self.verbalizer)))
    ...
    def generate_word_cloud(self, filename, top_tokens=50):
        ...
        top_indices = np.argsort(tensor_array)[::-1][:50]  # Adjust the number of tokens as needed
        ...
        wordcloud = WordCloud(
            width=450, height=450, background_color="white"
